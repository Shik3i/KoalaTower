...
import { BACKGROUNDS, type BackgroundTheme } from '../balance/backgrounds';
...
export class BackgroundRenderer {
	...
	constructor(stars: ReturnType<typeof import('./renderUtils').generateStars>, width: number, height: number, theme: BackgroundTheme = BACKGROUNDS[0]!) {
		this.width = width;
		this.height = height;
		this.stars = stars;
		this.theme = theme;
	...
	private drawStatic(): void {
		...
		// Deep space
		g.rect(0, 0, w, h).fill(c.deepSpace);

		// Atmospheric glow — two alternating theme hues.
		for (let i = 5; i >= 0; i--) {
			const t = i / 5;
			const r = w * 0.55 * (0.25 + t * 0.75);
			const alpha = 0.06 - t * 0.03;
			const col = i % 2 === 0 ? c.glowA : c.glowB;
			g.circle(cx, cy, r).fill({ color: col, alpha: Math.max(0.01, alpha) });
		}

		// Vignette — strength scaled per theme.
		for (let i = 0; i < 6; i++) {
			const t = i / 5;
			const r = w * (0.3 + t * 0.35);
			g.circle(cx, cy, r).fill({ color: 0x000000, alpha: t * 0.065 * this.theme.vignette });
		}
		...
		for (let x = 0; x <= w; x += gs) {
			grid.moveTo(x, 0).lineTo(x, h).stroke({ width: 0.5, color: c.grid, alpha: this.theme.gridAlpha });
		}
		for (let y = 0; y <= h; y += gs) {
			grid.moveTo(0, y).lineTo(w, y).stroke({ width: 0.5, color: c.grid, alpha: this.theme.gridAlpha });
		}
	}
	...
	update(time: number, _dt: number): void {
		...
		const starTint = this.theme.colors.star;
		for (let i = 0; i < this.stars.length; i++) {
			...
			gfx.clear();
			gfx.circle(star.x, star.y, star.size * (0.8 + twinkle * 0.4)).fill({ color: starTint, alpha: a });
			if (star.size > 1.4 && a > 0.45) {
				gfx.circle(star.x, star.y, star.size * 3.5).fill({ color: starTint, alpha: a * 0.12 });
			}
		}
	}
