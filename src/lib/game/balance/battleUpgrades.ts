...
/**
 * Field upgrade cost curve — "The Tower"-style.
 *
 * Every Field upgrade now uses the gentle `towerLike` curve with a near-linear
 * shape (costExponent 1.0) and a very small per-level compounding (costGrowth
 * ~1.01) so early levels stay cheap and big numbers arrive only deep in. The
 * anchor is Damage: buying level 36 costs ~$179, matching the original (where it
 * used to cost ~$12,000). `baseCost` is the only per-upgrade lever and scales
 * with how powerful one level of the upgrade is.
 */
