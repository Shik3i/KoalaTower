...
for (const sc of SCENARIOS) {
	const wsTot = totalWSLevels(sc.workshop) + totalWSLevels(sc.forge ?? {});
	const labTot = totalLabLevels(sc.labs);
	const runs: ReturnType<typeof simulateRun>[] = [];
	for (let i = 0; i < 3; i++) runs.push(simulateRun(sc.forge ?? {}, sc.workshop, sc.labs, 5000, sc.tier, sc.strategy));
	...
	console.log(`  Alloy:     ${Math.floor(avg('totalAlloyEarned')).toLocaleString()}`);
